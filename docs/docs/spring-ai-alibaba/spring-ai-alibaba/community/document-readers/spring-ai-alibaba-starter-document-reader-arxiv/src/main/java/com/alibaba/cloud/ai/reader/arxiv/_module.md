...
| 名称 | arxiv |
| 编码语言 | .java |
| 代码路径 | spring-ai-alibaba/community/document-readers/spring-ai-alibaba-starter-document-reader-arxiv/src/main/java/com/alibaba/cloud/ai/reader/arxiv |
| 包名 | spring-ai-alibaba.community.document-readers.spring-ai-alibaba-starter-document-reader-arxiv.src.main.java.com.alibaba.cloud.ai.reader.arxiv |
...
该流程图展示了`arxiv`目录下的层级关系，`arxiv`包含`client`文件夹和两个文件`ArxivDocumentReader.java`和`ArxivResource.java`。`client`文件夹下又包含多个文件，如`ArxivResult.java`、`ArxivSortOrder.java`等，清晰地展示了文件与文件夹之间的嵌套关系。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [ArxivResource.java](ArxivResource.md) | file | ArxivResource类管理Arxiv资源，支持查询、文档限制和文件处理。 |
| [ArxivDocumentReader.java](ArxivDocumentReader.md) | file | Arxiv文档读取器类，用于查询解析获取文档及元数据。 |
| [client](client/_module.md) | package | ArxivResult类管理论文元数据，ArxivSearch配置搜索参数，ArxivClient支持分页查询和PDF下载。 |

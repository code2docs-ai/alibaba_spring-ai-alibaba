...
| 名称 | alibaba |
| 编码语言 | .java |
| 代码路径 | spring-ai-alibaba/community/document-readers/spring-ai-alibaba-starter-document-reader-gitbook/src/main/java/com/alibaba |
| 包名 | spring-ai-alibaba.community.document-readers.spring-ai-alibaba-starter-document-reader-gitbook.src.main.java.com.alibaba |
...
该流程图展示了从`alibaba`到`gitbook`的层级关系，最终在`gitbook`目录下包含多个文件和子目录。`GitbookDocumentReader.java`、`GitbookClient.java`以及`model`目录下的`GitbookPage.java`和`GitbookSpace.java`文件均属于`gitbook`目录。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [cloud](cloud/_module.md) | package | Gitbook文档读取器通过API令牌和空间ID读取页面内容，支持自定义API URL和元数据配置。 |

...
| 名称 | toolcalling |
| 编码语言 | .java |
| 代码路径 | spring-ai-alibaba/community/tool-calls/spring-ai-alibaba-starter-tool-calling-jsonprocessor/src/main/java/com/alibaba/cloud/ai/toolcalling |
| 包名 | spring-ai-alibaba.community.tool-calls.spring-ai-alibaba-starter-tool-calling-jsonprocessor.src.main.java.com.alibaba.cloud.ai.toolcalling |
...
该流程图展示了`spring-ai-alibaba`项目中`tool-calls`模块的层级结构。`toolcalling`作为根目录，包含`jsonprocessor`子目录，而`jsonprocessor`目录下则包含多个Java文件，如`JsonAutoConfiguration.java`、`JsonInsertService.java`等。这些文件共同构成了JSON处理功能的核心实现。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [jsonprocessor](jsonprocessor/_module.md) | package | JsonAutoConfiguration类配置Gson服务，实现JSON字段的插入、解析、移除和替换操作。 |

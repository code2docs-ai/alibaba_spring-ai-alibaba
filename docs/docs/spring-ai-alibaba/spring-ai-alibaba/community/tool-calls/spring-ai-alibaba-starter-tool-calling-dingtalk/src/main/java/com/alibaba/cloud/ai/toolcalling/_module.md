...
| 名称 | toolcalling |
| 编码语言 | .java |
| 代码路径 | spring-ai-alibaba/community/tool-calls/spring-ai-alibaba-starter-tool-calling-dingtalk/src/main/java/com/alibaba/cloud/ai/toolcalling |
| 包名 | spring-ai-alibaba.community.tool-calls.spring-ai-alibaba-starter-tool-calling-dingtalk.src.main.java.com.alibaba.cloud.ai.toolcalling |
...
该流程图展示了 `toolcalling` 目录下的 `dingtalk` 文件夹及其包含的四个 Java 文件。`dingtalk` 文件夹作为 `toolcalling` 的子目录，包含了与钉钉工具调用相关的配置、服务和工具类文件。这些文件共同构成了钉钉工具调用的核心功能模块。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [dingtalk](dingtalk/_module.md) | package | 钉钉机器人配置类管理认证参数，签名工具生成安全URL，服务类处理消息发送，配置类自动启用服务。 |

...
| 名称 | time |
| 编码语言 | .java |
| 代码路径 | spring-ai-alibaba/community/tool-calls/spring-ai-alibaba-starter-tool-calling-time/src/main/java/com/alibaba/cloud/ai/toolcalling/time |
| 包名 | spring-ai-alibaba.community.tool-calls.spring-ai-alibaba-starter-tool-calling-time.src.main.java.com.alibaba.cloud.ai.toolcalling.time |
...
该流程图展示了`time`目录下的四个Java文件：`GetCurrentLocalTimeService.java`、`GetCurrentTimeByTimeZoneIdService.java`、`ZoneUtils.java`和`TimeAutoConfiguration.java`。这些文件均位于同一层级，直接归属于`time`目录，没有进一步的子目录结构。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [TimeAutoConfiguration.java](TimeAutoConfiguration.md) | file | 启用时间服务需相关类存在且属性为真。 |
| [GetCurrentTimeByTimeZoneIdService.java](GetCurrentTimeByTimeZoneIdService.md) | file | 根据时区ID获取当前时间，记录请求和响应。 |
| [ZoneUtils.java](ZoneUtils.md) | file | ZoneUtils类用ZoneId获取并格式化指定时区当前时间。 |
| [GetCurrentLocalTimeService.java](GetCurrentLocalTimeService.md) | file | GetCurrentLocalTimeService类用于获取当前本地时间。 |
